...
import (
	"bch-mapping-contract/contract/blocklist"
	...
	_ "bch-mapping-contract/sdk" // ensure sdk is imported
	"encoding/hex"
	"strconv"
	"strings"
...
func SetMaxUnmapPerBlock(input *string) *string {
	checkAdmin()
	if input == nil || *input == "" {
		ce.CustomAbort(ce.NewContractError(ce.ErrInput, "expected satoshis-per-block as integer string"))
	}
	v, err := strconv.ParseInt(strings.TrimSpace(*input), 10, 64)
	if err != nil || v < 0 {
		ce.CustomAbort(ce.NewContractError(ce.ErrInput, "expected non-negative integer satoshis-per-block"))
	}
	...
	return mapping.StrPtr("max unmap per block set to " + strconv.FormatInt(v, 10) + " satoshis")
}
...
// Withdraws BCH from the caller's own balance to a Bitcoin Cash address.
...
// Withdraws BCH from a third-party account that has approved the caller.
...
func GetInfo(_ *string) *string {
	return mapping.StrPtr(`{"name":"Bitcoin Cash","symbol":"BCH","decimals":"8"}`)
}
