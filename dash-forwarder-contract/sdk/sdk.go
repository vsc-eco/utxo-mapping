...
// ContractCallAs invokes contractId.method(payload) with the callee's
// effectiveCaller set to effectiveCallerDID. Wraps the contracts.call_as
// host function added in workstream 2 (modules/wasm/sdk/sdk.go).
//
// Permission: this contract MUST appear in system-config.TrustedForwarders,
// or the host function returns a permission-denied error. The error is
// transparently surfaced as a *string with the error message — callers
// should check for an empty / "ABORT" prefix and propagate as needed.
//
// effectiveCallerDID is the identity the callee should treat as the
// authorising actor — typically a DashDID like
//
//	did:pkh:bip122:00000bafbc94add76cb75e2ec9289483:y...
//
// Per-call-frame only: the callee's own outbound calls don't propagate
// the override. ERC-2771 semantics.
func ContractCallAs(contractId string, method string, payload string, effectiveCallerDID string, options *ContractCallOptions) *string {
	optStr := ""
	if options != nil {
		optByte, err := tinyjson.Marshal(options)
		if err != nil {
	...
	return contractCallAs(&contractId, &method, &payload, &optStr, &effectiveCallerDID)
}
