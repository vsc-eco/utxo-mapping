...
type BlockData struct {
	BlockMap   map[uint32]BlockHeaderBytes // maps block heights to raw 80 byte block headers
	LastHeight uint32
}
...
func (bd *BlockData) HandleAddBlocks(rawHeaders []BlockHeaderBytes) error {
	lastBlockBytes := bd.BlockMap[bd.LastHeight]
	...
	for _, headerBytes := range rawHeaders {
		...
		bd.BlockMap[blockHeight] = headerBytes
