...
import (
	"doge-mapping-contract/contract/blocklist"
	...
	_ "doge-mapping-contract/sdk" // ensure sdk is imported
	"encoding/hex"
	"strconv"
	"strings"
...
func checkOracle() {
	...
	// oracle shortcut applies on real testnet + regtest.
...
func SeedBlocks(blockSeedInput *string) *string {
	...
	// Idempotency relaxation: testnet + regtest can re-seed.
...
func SetMaxUnmapPerBlock(input *string) *string {
	checkAdmin()
	if input == nil || *input == "" {
		ce.CustomAbort(ce.NewContractError(ce.ErrInput, "expected dogetoshis-per-block as integer string"))
	}
	v, err := strconv.ParseInt(strings.TrimSpace(*input), 10, 64)
	if err != nil || v < 0 {
		ce.CustomAbort(ce.NewContractError(ce.ErrInput, "expected non-negative integer dogetoshis-per-block"))
	}
	...
	return mapping.StrPtr("max unmap per block set to " + strconv.FormatInt(v, 10) + " dogetoshis")
}
...
// Withdraws DOGE from the caller's own balance to a Dogecoin address.
...
// Withdraws DOGE from a third-party account that has approved the caller.
...
func GetInfo(_ *string) *string {
	return mapping.StrPtr(`{"name":"Dogecoin","symbol":"DOGE","decimals":"8"}`)
}
