...
// `execute` entrypoint that the BTC mapping contract calls during
// swap-tagged deposits. When the router fails or returns
// `amount_out == "0"`, btc-mapping-contract previously reverted the
...
import (
	...
	_ "btc-mapping-contract/sdk"
)

func main() {}

// Always returns {"amount_out":"0", "pool_state":{...}} regardless of
// input. Drives btc-mapping-contract into the BTC-C4 router-failure
