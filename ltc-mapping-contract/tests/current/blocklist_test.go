...
// LTC skips PoW validation (Scrypt not in btcsuite), so any valid-format headers
...
func TestAllOperations(t *testing.T) {
	...
	t.Run("AddBlocks_ValidChainSucceeds", func(t *testing.T) {
		// LTC skips PoW validation, so valid-format headers with correct
	...
	t.Run("Unmap_ZeroAmountFails", func(t *testing.T) {
		payload := `{"amount":0,"to":"ltc1qw508d6qejxtdg4y5r3zarvary0c5xw7kgmn4n9"}`
