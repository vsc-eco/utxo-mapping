// Mock router contract for BTC-C4 (propagated to dash) test fixture.
//
// The real DEX router (vsc-eco/dex-contracts/dex-router-v2) exposes an
// `execute` entrypoint that the Dash mapping contract calls during
// swap-tagged deposits. When the router fails or returns
// `amount_out == "0"`, dash-mapping-contract previously reverted the
// entire map transaction — losing the user's deposit credit. The
// BTC-C4 (propagated to dash) fix instead refunds the depositor with raw BTC.
//
...
import (
	...
	_ "dash-mapping-contract/sdk"
)

func main() {}

// Always returns {"amount_out":"0", "pool_state":{...}} regardless of
// input. Drives dash-mapping-contract into the BTC-C4 (propagated to dash) router-failure
