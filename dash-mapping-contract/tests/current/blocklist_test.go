...
import (
	btcMapping "dash-mapping-contract"
...
func TestAllOperations(t *testing.T) {
	...
	t.Run("GetInfo_ReturnsTokenMetadata", func(t *testing.T) {
		...
		assert.JSONEq(t, `{"name":"Dash","symbol":"DASH","decimals":"8"}`, r.Ret)
	})
	...
	// Pentest finding BTC-C2 (propagated to dash-mapping): replaceBlock /
	// replaceBlocks overwrite the block header but leave the per-height
	// observed-tx list untouched. After a reorg that orphaned a deposit's
	// containing block, the observed entry persists, so the deposit credit
	// remains as if the L1 tx were still in the canonical chain. Fix:
	// delete ObservedBlockPrefix+height in both handlers.
